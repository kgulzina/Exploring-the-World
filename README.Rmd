...
```{r}
# (trial) this will return list with 11 lists
...
#   df:     data frame
    ...
    for (f in 1:length(spf$geometry)) {
        for (i in 1:length(spf$geometry[[f]])) {
            temp <- map_depth(spf$geometry[[f]][[i]], 1, data.frame)
...
## here we lost some tiny parts of Australia :D
```
